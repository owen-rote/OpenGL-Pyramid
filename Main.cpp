...
int main() {
	...
	// Setup OpenGL Version 3
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);

	// Create 800*800 window
	...
	gladLoadGL();
	glViewport(0, 0, width, height); // OpenGL will render from (0,0) to (800,800)
	...
	// Unbind to prevent accidently modifying
	VAO1.Unbind();
	VBO1.Unbind();
	EBO1.Unbind();
	...
	while (!glfwWindowShouldClose(window)) {
		...
		VAO1.Bind(); // Bind so OpenGL knows to use it
